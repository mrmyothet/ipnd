...
easy_paragraph = '''___1___ is a widely used high-level, general-purpose, interpreted, dynamic ___2___ language.
                [22][23] Its design philosophy emphasizes code readability, and its ___3___ allows programmers to
                express concepts in ___4___ lines of code than would be possible in languages such as C++ or Java.
                [24][25] The language provides constructs intended to enable clear programs on both a small and large
                scale.[26] Source: Wikipedia'''
medium_paragraph = '''Python supports multiple programming ___1___, including object-oriented, imperative
                   and ___2___ programming or procedural styles. It features a dynamic type system and automatic
                    ___3___ management and has a large and comprehensive standard ___4___.[27] Source: Wikipedia'''
hard_paragraph = '''Python interpreters are available for installation on many operating systems, allowing Python
                    code execution on a wide variety of systems. Using third-party tools, such as ___1___ or ___2___,
                    [28] Python code can be ___3___ into stand-alone executable programs for some of the most popular
                    operating systems, allowing the distribution of Python-based software for use on those environments
                    without requiring the installation of a Python interpreter. ___4___, the reference implementation
                    of Python, is free and open-source software and has a community-based development model, as do
                     nearly all of its alternative implementations. Source: Wikipedia'''
...
def greeting(player_name):
	...
	print( "\nGreat! Welcome, " + player_name + ". The purpose of this game is to fill in the blanks for all \
    the sentences provided. Now, please pick a level by entering in the appropriate number: ")
...
def replace_the_blank(word, replaced, blanks, user_answer, index):
	...
	the 'user_answer' for that blank, the index number of that 'user_answer' to correctly match that with the
    right blank to fill in.
...
def fill_in_answers(paragraph, blanks, replaced, user_answer, index):
	...
	Inputs: Paragraph from level, 'blanks' list, the replaced paragraph that has the correct answers so far,
    if applicable,
	the 'user_answer' for that blank, the index number of that 'user_answer' to correctly match that with
    the right blank to fill in.
	Outputs: The correctly replaced paragraph."""
	...
	head, sep, tail = replaced.partition("Wikipedia") # To get rid of the extra 'replacements'
                                                      #that tacked on to the end of every paragraph. Inputs:
                                                      # The replaced paragraph.  Outputs: Cleaned replaced paragraph.
	replaced = head + sep
	return replaced
...
def collect_answers(level, paragraph, answers):
	...
	for blank in blanks:
		...
		while user_answer != answers[index]:
			...
			user_answer = input("Type it here again: ")
			user_answer = user_answer.lower()
...
def play_game():
	...
	player_name = input("What is the name of the current player? ")
